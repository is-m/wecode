...
public class PropertyService extends BaseService<Property, PropertyDto, PropertyResultDto> {
  ...
  public PropertyService(JpaRepository<Property, String> repository, BaseMapper<Property, PropertyDto, PropertyResultDto> mapper) {
  ...
  public PropertyResultDto create(@Validated({}) PropertyDto dto) {
    ...
    if (PropertyConstant.VALUE_TYPE_item.equals(property.getValueType())) {
      Property parentProperty = repo.getOne(dto.getPid());
  ...
  private String generatePath(Property property) {
    ...
    Property parentProperty = repo.getOne(pid);
  ...
  public Page<PropertyResultDto> findByPage(Pageable pageable, PropertyDto condition) {
    return repo.findAll(Example.of(mapper.to(condition)), pageable).map(mapper::from);
  }
  ...
  public PropertyResultDto findOne(PropertyDto condition) {
    return mapper.from(repo.findOne(Example.of(mapper.to(condition))).get());
  }
  ...
  public List<PropertyResultDto> findChildrenByPath(String parentPath, boolean deepSearch) {
    ...
    return MapperUtils.from(deepSearch ? repo.findByPathLike(parentPath + ".") : repo.findByPid(parent.getId()), mapper);
  }
  ...
  public List<PropertyResultDto> findPropertyTree() {
    List<Property> entities =
        repo.findByValueTypeIn(new String[] {PropertyConstant.VALUE_TYPE_simple, PropertyConstant.VALUE_TYPE_blend}, Sort.by("seq"));
  ...
  public void delete(String... ids) {
    ...
    container.values().forEach(delSet::addAll);
    repo.deleteAll(delSet);
  }
