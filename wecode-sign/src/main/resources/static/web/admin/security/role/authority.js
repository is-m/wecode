...
pageContext.controller("admin.security.role.authority", ["rt/request"], function (page, $S, http) {
    ...
    page.init = function (record) {
        ...
        role = record;
        // 加载角色栏目，权限数据
        http.doGet("/services/security/role/{0}/permissionIds".format(role.id)).done(function (permissionIds) {
            $("#treeFunction").xWidget().setCheckedValues("id", permissionIds);
            // 加载完权限后，根据权限来影响栏目的选项
            var treeCatelog = $("#treeCatelog").xWidget();
            var undisabledNodes = treeCatelog.getAllNodes().filter(function (item) {
                return item.allowType != "authority";
            });
            treeCatelog.setCheckNode(undisabledNodes.filter(function (item) {
    ...
    page.sync = function () {
        // 将系统所有权限获取，并写入到权限表
        http.doPost("/services/security/permission/sync").done(
            function (res) {
    ...
    page.clear = function () {
        http.doDelete("/services/security/permission/clearInvalid").done(function (res) {
            $("#treeCatelog").xWidget().reload();
            $("#treeFunction").xWidget().reload();
        });
    };
    ...
    page.save = function () {
        // 获取选中的权限数据
        var checkedData = $("#treeFunction").xWidget().getCheckedNodes().filter(function (item) {
            return item.type == "operate"
        });
