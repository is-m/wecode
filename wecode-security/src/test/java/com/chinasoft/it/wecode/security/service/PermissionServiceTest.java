...
public class PermissionServiceTest {
	...
	public void test01CreatePermission() {
		...
		permissionDto = new PermissionDto();
		permissionDto.setCode("1111111111111111111111111111111111111111111111111111111111111111111111111111111111111111SYS::Test::test01CreatePermission" + System.currentTimeMillis());
		permissionDto.setNote("测试创建权限点3");
		permissionDto.setType(SecurityConstant.PERMISSION_TYPE_MODULE);
		permissionDto.setPid("111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111");
		create = service.create(permissionDto);
		System.out.println(create);
	}
	...
	public void test01PattralCreatePermission() {
		...
		for(int i=1;i<11;i++) {
			...
			if(i%5==0) {
				dto.setPid("111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111");
			}
			waitCreateList.add(dto);
		}
		/*waitCreateList.parallelStream().forEach(item->{
			service.create(item);
		});*/
